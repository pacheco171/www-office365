...
function renderCompare(){
  ...
  if(!el)return;
  // Só renderiza se o painel estiver aberto
  var panel=document.getElementById('comparePanel');
  if(panel&&panel.style.display==='none')return;
  if(snapshots.length<2){el.innerHTML='<div style="text-align:center;padding:24px;color:var(--muted);font-size:13px">Necessário pelo menos 2 snapshots para comparar.</div>';return;}
  ...
  const cardHtml=(snap,counts)=>`
    <div class="compare-card">
      <div class="compare-header">
        <div><div class="compare-month">${snap.label}</div><div style="font-size:11px;color:var(--muted);margin-top:3px">${snap.data.length} usuários</div></div>
        <div class="compare-total">${fmtBRL(snapshotCost(snap))}</div>
      </div>
      ${LICENSES.filter(l=>counts[l.id]&&l.id!=='none').sort((a,b)=>(counts[b.id]||0)*b.price-(counts[a.id]||0)*a.price).map(l=>`
        <div class="compare-row-item">
          <span class="cri-name">${licBadge(l.id)}</span>
          <span class="cri-qty">${counts[l.id]} usuários</span>
  ...
  const diffRows=allIds.map(id=>{
    ...
    if(dq===0)return null;
    const reason=dq>0?`+${dq} usuário${Math.abs(dq)>1?'s':''} adicionado${Math.abs(dq)>1?'s':''}`:`${dq} usuário${Math.abs(dq)>1?'s':''} removido${Math.abs(dq)>1?'s':''}`;
  ...
  el.innerHTML=`
    <div style="font-size:10px;color:var(--muted);letter-spacing:2px;text-transform:uppercase;font-weight:700;margin-bottom:12px">Comparativo ${s1.label} × ${s2.label}</div>
    <div class="compare-row">${cardHtml(s1,m1)}${cardHtml(s2,m2)}
      ${diffRows.length?`<div class="diff-card">
        <div class="diff-title">Variações — ${s1.label} → ${s2.label} · Total: ${deltaBRLBadge(c2-c1)}</div>
        ${diffRows.join('')}
      </div>`:''}
    </div>`;
}
